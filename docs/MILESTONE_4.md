...
Order: **A → C → B1 → B2** (proposed — tracked in [#11](https://github.com/changbaebang/career-radar/issues/11)). M4-C depends on no model behaviour, while B2 is the first prompt change since M1 and waits for the live measurements in #4.
...
- Open design decisions from the #7 review, each with a reviewer recommendation but not yet settled: [#8](https://github.com/changbaebang/career-radar/issues/8) evidence validation timing (before M4-B1), [#9](https://github.com/changbaebang/career-radar/issues/9) reference validation rule and failure handling (before M4-B1), [#10](https://github.com/changbaebang/career-radar/issues/10) event store shape (before M4-C), [#11](https://github.com/changbaebang/career-radar/issues/11) slice order. The sections above state the proposals; the issues hold the final call.
