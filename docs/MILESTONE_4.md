...
Order: **A → C → B1 → B2**, confirmed by the owner on 2026-09-11 ([#11](https://github.com/changbaebang/career-radar/issues/11)). M4-C depends on no model behaviour, while B2 is the first prompt change since M1 and waits for the live measurements in #4. A temporarily unused B1 contract is acceptable; lack of API funding does not waive B2's live gate.
