...
- **Confirmed 2026-09-11 — [#9](https://github.com/changbaebang/career-radar/issues/9): located exact-match validation.** Reuse M1 `normalizeEvidence` only after resolving an allowlisted source/path; matching text elsewhere is insufficient. Both sources are required. Missing evidence or any failed reference coerces seniority/career-story judgments to `uncertain` with neutral text and low confidence; failed individual risks are excluded and recorded in `unknowns`. Candidate scope comparisons cannot rely solely on titles/headlines/skill tokens. Text/location checks do not establish semantic truth or prove absence of demographic-proxy reasoning; B2 still needs producer evals.
