...
**Status:** M4-A merged in #12 (`f097295`). M4-C stage-aware feedback implemented for review; screening context remains proposed. Scope: private, single-owner use. [M4-A notes](MILESTONE_4A.md) separate policy-contract verification from pending human fit-label review; [M4-C notes](MILESTONE_4C.md) define history replacement, stage/date coverage and compatibility. No live model calls or real user data were used for M4-C.
...
- **Proposed — tracked in [#9](https://github.com/changbaebang/career-radar/issues/9): reuse the M1 grounding rule.** Candidate quotes are checked with the existing `isGrounded` normalization (lower-case, collapsed whitespace, trailing punctuation stripped, exact match against the candidate evidence pool). Job quotes are checked the same way against `description`, `required[].text`, `preferred[].text` and `responsibilities[]`. `seniorityFit` and `careerStoryRisk` values without validated candidate **and** job references are coerced to `uncertain`. Individual `screeningRisks` entries have no `uncertain` severity in the proposed schema (#9): the contract must either add one or define that a risk with failed references is removed and recorded in `unknowns` — never kept as a confident risk and never dropped silently. Also (#9): `isGrounded` compares quote text only; validating `EvidenceRef.path` against the located source is new work in M4-B1, not something the M1 rule already covers.
...
- **Proposed — tracked in [#8](https://github.com/changbaebang/career-radar/issues/8): references are validated at write time only.** Assessment snapshots hold `profileId`, not the profile body, and `profile_upsert` overwrites, so a candidate-side reference cannot be re-opened later. M4-B1 adds an input-version identifier to the snapshot; it does not add a per-assessment profile copy. Note (#8): the existing `sourceHash` hashes the raw resume text, so the same text re-extracted under another prompt/model version yields the same hash — the identifier must cover the structured profile (for example a hash of the stored profile JSON plus its extraction `promptVersion`), not only the source text. Replay and reproducibility come from the synthetic eval fixtures in M4-A, not from stored personal profiles.
...
- Still open: [#8](https://github.com/changbaebang/career-radar/issues/8) evidence validation timing and [#9](https://github.com/changbaebang/career-radar/issues/9) reference validation rules (before B1). Owner-confirmed on 2026-09-11: [#10](https://github.com/changbaebang/career-radar/issues/10) existing history-store extension and [#11](https://github.com/changbaebang/career-radar/issues/11) A → C → B1 → B2 order; tracked through the M4-C PR.
