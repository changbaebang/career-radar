...
**Status:** M4-A merged in #12 (`f097295`), M4-C merged in #13 (`5fe22ba`). M4-B1 contract/validators implemented for review after owner approval of #8/#9; model/UI integration remains B2. Scope: private, single-owner use. [M4-A notes](MILESTONE_4A.md) separate policy-contract verification from pending human fit-label review; [M4-C notes](MILESTONE_4C.md) define history semantics; [M4-B1 notes](MILESTONE_4B1.md) define reference/identity limits. No live model calls or real user data were used for these slices.
...
| Outcomes | M4-C: normalized stage, occurrence time, provenance, versioned minimal events and whole-history replacement | Targeted single-event corrections and private-history import remain out of scope |
| Summary | M4-C: current verdict × stage, explicit reported stage reach and role progression; distinct IDs, coverage/unknowns and last-update exclusions | Cohort/occurrence-date reports and direct-role/transition classification are deferred |
| Snapshots | Job and assessment snapshot, profile ID; B1 adds structured profile/JD hashes without another profile copy | Original personal profile and extraction provenance remain unavailable for replay |
| Evals | M4-A: 28 synthetic policy contracts (16 original + 12 variants), versioned reports and contract/annotation comparison; M4-C: separate G/H outcome-analytics tests | Human-reviewed fit labels (all 28 pending), model mode and screening-context cases |
...
B1 exports an opt-in `ScreeningAssessmentSchema` with optional versioned `screeningContext`; existing `FitAssessmentSchema` and tool outputs stay unchanged until B2. There is no second fit score. Absence on historical results means **not evaluated**, not low risk. Strict Zod schemas and consumers must be updated together; adding an optional field does not make an old strict parser forward-compatible. See [B1 contract](MILESTONE_4B1.md) for bounds, paths and validation limits.
...
- **Confirmed 2026-09-11 — [#9](https://github.com/changbaebang/career-radar/issues/9): located exact-match validation.** Reuse M1 `normalizeEvidence` only after resolving an allowlisted sentence-level source/path (whole-description quoting is not an anchor); matching text elsewhere is insufficient. Both sources are required. Missing evidence or any failed reference coerces seniority/career-story judgments to `uncertain` with neutral text and low confidence; failed individual risks are excluded and recorded in `unknowns`. Candidate scope comparisons cannot rely solely on titles/headlines/skill tokens. Text/location checks do not establish semantic truth or prove absence of demographic-proxy reasoning; B2 still needs producer evals.
...
- **Confirmed 2026-09-11 — [#8](https://github.com/changbaebang/career-radar/issues/8): write-time validation, no extra profile copy.** B1 snapshots add `inputIdentity` (`structured-input-v1`, canonical structured `profileHash` and `jobHash`) from the captured assessment inputs. Raw-resume `sourceHash` alone is insufficient. Identical structured content from different extraction runs intentionally shares an identity: the current profile has no extraction prompt/model provenance. Hashes neither retain missing inputs nor anonymize them. Old snapshots are not backfilled from mutable profiles; reproducible replays use synthetic fixtures. The validator is opt-in in B1 and must be wired before context persistence/output in B2.
...
**Confirmed 2026-09-11 — [#10](https://github.com/changbaebang/career-radar/issues/10):** extend existing `application_events`, not a second history store. V2 JSON contains only application ID, revision, status, normalized stage, provenance, recording time and optional occurrence time. It no longer copies the application record. V1 JSON is preserved and read conservatively with stage/date unknown and `legacy_mapping`; only the latest legacy snapshot is used when no explicit reports exist in the active revision. Migration 2 adds an index without backfilling personal data. `clear`/`db:reset` covers every revision.

M4-C uses explicit `historyMode`: `append` retains independent progression reports; `replace` supersedes the entire previous outcome history for aggregate purposes while keeping audit rows. The default is `append`; `replace` must be requested explicitly. This is not targeted single-event correction: clarify the whole-history scope before using it. Notes-only edits retain history. Clearing stage supersedes prior history even in append mode. Occurrence time is never inferred from recording time; omitted dates reset to unknown on a status/stage change and otherwise remain unchanged. See [M4-C contract](MILESTONE_4C.md).
...
| M4-B1 — screening context contract | Opt-in versioned context schema, located reference validator and `uncertain` coercion (§4), structured `inputIdentity` on snapshots, A–F authored contract tests. **No prompt or UI change, and the server does not persist/emit context yet** | New/legacy payload tests, unchanged base fit, no inferred judgments; location checks are not semantic/proxy-detection guarantees; synthetic only |
| M4-B2 — screening context in the model path | Prompt integration, prompt/schema version bump, next widget URI (v5 or later; M4-C uses v4) with host descriptor refresh, readable uncertainty UI | Starts only after #4 has measured the live path; synthetic versus live results explicitly separated; real local browser smoke |

Order: **A → C → B1 → B2**, confirmed by the owner on 2026-09-11 ([#11](https://github.com/changbaebang/career-radar/issues/11)). M4-C depends on no model behaviour, while B2 is the first prompt change since M1 and waits for the live measurements in #4. A temporarily unused B1 contract is acceptable; lack of API funding does not waive B2's live gate. Owner decision 2026-09-11: the B2 code (prompt, pipeline, v5 widget) is implemented and verified synthetically ([MILESTONE_4B2.md](MILESTONE_4B2.md)); its live gate (model-mode A–F evals, #4 measurement, host smoke) stays open until cost approval and is not claimed by the merge.
...
- [#4: live model and 90-second batch budget](https://github.com/changbaebang/career-radar/issues/4): execute only after explicit cost/transmission approval. Measure actual completion/cancellation/cost, not mock-call savings. A different provider proves only its own path (`pnpm measure:live --provider openrouter` records `openrouter/<model>@<upstream>`, see [PROVIDERS.md](PROVIDERS.md)). Harness: `pnpm measure:live` ([LIVE_MEASUREMENT.md](LIVE_MEASUREMENT.md)), dry run by default, live only with `--approve-network --approve-model-cost` plus a typed call cap; three approved runs before any deadline decision.
...
- Owner-confirmed on 2026-09-11: [#8](https://github.com/changbaebang/career-radar/issues/8) write-time validation/structured input identity and [#9](https://github.com/changbaebang/career-radar/issues/9) located exact match/explicit uncertainty, tracked through the B1 PR. [#10](https://github.com/changbaebang/career-radar/issues/10) existing history-store extension and [#11](https://github.com/changbaebang/career-radar/issues/11) A → C → B1 → B2 order were implemented in #13. #4–6 remain separate live/host/public gates.
