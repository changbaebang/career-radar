...
**Report.** `reportKind: "model-evaluation"`, report version 3, `model-metrics-v1`, validated
...
hard blockers, citations) next to the final counts (`final`) and the pipeline stages that actually
acted on it, as short codes (`notes`: `ungroundedMatchRemoved`, `citationRekeyed`, `citationOrphan`,
`citationInvalid`, `screeningContextDiscarded`). The codes come from what each stage reports it did
(`finalizeAssessmentDetailed`), never from the fixed sentences in `missingInformation`: the model's
own entries there are kept verbatim, so a sentence match could not prove the stage ran. Notes added
