...
4. Request a maximum number of REALISTIC/STRETCH roles (at most five combined) and optional PASS explanations. Missing results stay missing. Failures are not PASS. The first failure stops further model calls; successful items remain available. Same-label ranking uses confidence, contortion, then stable ID—not scores or hiring odds.
