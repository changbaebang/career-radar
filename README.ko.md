...
4. REALISTIC/STRETCH 개수(합계 최대 5)와 PASS 설명 여부를 요청합니다. 부족한 수를 채우려고 판정을 바꾸지 않습니다. 분석 실패는 PASS가 아니며 첫 실패 후 나머지 호출을 멈춥니다. 성공한 결과는 유지합니다. 같은 판정 안에서는 confidence → contortion → ID 순으로 정렬하며 합격 확률·점수 순위가 아닙니다.
