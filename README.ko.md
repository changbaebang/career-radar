...
### M4-B1 근거 계약 — 아직 모델·화면 기능은 아님

선택적으로 사용할 서류 검토 맥락 스키마와 출처·위치별 근거 검증기를 구현했습니다.
근거가 부족하거나 틀리면 불확실성으로 남기며, 기존 판정·추천 순위는 바꾸지 않습니다.
현재 MCP 응답에는 새 필드가 나오지 않습니다. 프롬프트·위젯 연결은 별도 승인된
라이브 측정 이후 B2에서 진행합니다. 새 판정 스냅샷에는 실제 구조화 프로필·JD의
해시만 추가하고 프로필 사본은 늘리지 않습니다. 기존 스냅샷은 그대로 읽으며
현재 프로필로 해시를 소급 생성하지 않습니다. 해시는 사라진 입력을 복원하거나
추출 실행 이력을 증명하지 않습니다. [B1 계약과 한계](docs/MILESTONE_4B1.md).
